...
export function HealthBar(props) {
    ...
    function Segment(segProps) {
        ...
        return <path
            className={props.health < 25 ? "fill-red-600" : !isCT ? 'fill-southLanGreen' : 'fill-southLanPurple'}
    ...
    return <div className={props.className} style={{ width: barLength, ...props.style }}>
        <svg
            className={`absolute bottom-0 ${props.health < 100 / numSegments || isCT ? 'fill-white' : 'fill-black'}`}
            height={barHeight}
            ...
            <g id="layer1">
                ...
                <image
                    ...
                    y={-barHeight / 2}
                    className={`${!isCT && props.health > 100 / numSegments ? 'brightness-0' : 'brightness-100'}`}
                ...
                />
                <text
                    className={`font-bold ${props.health < 100 / numSegments || isCT ? 'fill-white' : 'fill-black'}`}
