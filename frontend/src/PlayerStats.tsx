...
export function PlayerStats(props) {
    ...
    return <div className="grid grid-cols-2 grid-rows-5 w-screen absolute bottom-0 overflow-clip">
        {players.map((player: any) => <PlayerBar key={player.observer_slot} data={player} round={props.round} roundPhase={props.roundPhase} roundDamage={props.roundDamage} isSpecced={props.currentSpec['steamid'] === player['steamid']}/>)
        }
    </div>
}
